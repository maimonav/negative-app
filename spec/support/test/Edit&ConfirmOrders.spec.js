...
describe("edit&confirmOrder", () => {
    beforeEach(() => {
        ...
        inventoryManagement.orders.set(
    ...
    it("UnitTest-editOrder in Order class", async() => {
        ...
        expect(
            await orderForCafeteriaProduct.editOrder(
                new Date(1992, 6, 9), -3,
                productList
            )
        ).toBe("The order edited successfully completed");
        expect(orderForCafeteriaProduct.date).toEqual(new Date(1992, 6, 9));
        expect(orderForCafeteriaProduct.supplierId).toBe(-3);
        expect(
            orderForCafeteriaProduct.productOrders.get(product.id).expectedQuantity
        ).toBe(7);
        productList = [];
        expect(
            await orderForCafeteriaProduct.editOrder(
        ...
        expect(orderForCafeteriaProduct.productOrders.size).toBe(0);
        let movieList = [
            { id: movie.id, quantity: 7, key: -3, examinationRoom: -4 },
        ];
        expect(await orderFormovie.editOrder(null, null, movieList)).toBe(
            "The order edited successfully completed"
        );
    ...
    it("UnitTest-confirmOrder in Order class", async() => {
        //confirm for product
        let productList = [];
        expect(await orderForCafeteriaProduct.confirmOrder(productList)).toBe(
        ...
        expect(await orderFormovie.confirmOrder(productList)).toBe(
            "Order confirmation success"
        );
    ...
    it("UnitTest-editOrder in cinemaSystem class", async() => {
        ...
        ).toBe(cinemaSystem.inappropriatePermissionsMsg);
        expect(
            await cinemaSystem.editOrder(
                orderForCafeteriaProduct.id,
                new Date(1992, 6, 9), -3,
    ...
    it("UnitTest-editOrder in ServiceLayer class", async() => {
        ...
        ).toBe(" The order " + dummyNumber + " doesn't exists");
        ...
        ).toBe("The product " + dummyNumber + " doesn't exists");
        ...
        ).toBe("The user performing the operation does not exist in the system");
        expect(
            await serviceLayer.editOrder(
                orderForCafeteriaProduct.name,
                new Date(1992, 6, 9), -3,
    ...
    it("Integration-confirmOrder in InventoryManagement class", async() => {
        ...
        expect(
            await inventoryManagement.confirmOrder(orderFormovie.id, movieList)
        ).toBe("Order confirmation success");
        expect(orderFormovie.productOrders.get(movie.id).actualQuantity).toBe(1);
        expect(movie.movieKey).toBe(-3);
        expect(movie.examinationRoom).toBe(-4);
    ...
    it("Integration-confirmOrder in cinemaSystem class", async() => {
        ...
        expect(
            await cinemaSystem.confirmOrder(orderFormovie.id, movieList, manager.id)
        ).toBe("Order confirmation success");
        expect(orderFormovie.productOrders.get(movie.id).actualQuantity).toBe(1);
        expect(movie.movieKey).toBe(-3);
        expect(movie.examinationRoom).toBe(-4);
    ...
    it("Integration-confirmOrder in ServiceLayer class", async() => {
        ...
        ).toBe("The product " + product.name + "'s quantity received is not a number type");
        let movieList = [
            { name: movie.name, examinationRoom: -4 },
        ];
        expect(
            await serviceLayer.confirmOrder(
                orderFormovie.name,
        ...
        ).toBe("Not all parameters required for product or film approval were accepted");
        movieList = [
        ...
        expect(movie.movieKey).toBe(-3);
        expect(movie.examinationRoom).toBe(-4);
        expect(await serviceLayer.editOrder(orderFormovie.name, null, null, movieList, manager.userName)).toBe('Edit of order ' + orderFormovie.name + ' fail- You cannot edit a previously approved order.');
        expect(await serviceLayer.confirmOrder(orderFormovie.name, movieList, manager.userName)).toBe('Confirm of order ' + orderFormovie.name + ' fail- You cannot re-confirm order.');
