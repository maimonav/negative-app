...
describe("MovieOrder Operations Tests", () => {
    ...
    it('UnitTest addMovieOrder  - Service Layer', async () => {
        ...
        //Input validation
        await validate(serviceLayer, serviceLayer.addMovieOrder, { 'Order ID ': 'Order', 'Date ': 'date','Supplier Name ':'Supplier','Movies List ':'["Movie"]', 'Username ': 'User' })
        
        serviceLayer.orders.set("Order", 1);
        let result = await serviceLayer.addMovieOrder('Order','date','Supplier', '["Movie"]', 'User');
        expect(result).toBe("The order already exist");
        serviceLayer.orders=new Map();
        result = await serviceLayer.addMovieOrder('Order','date','Supplier', '["Movie"]', 'User');
    ...
    it('UnitTest removeOrder - Service Layer', async () => {
        let serviceLayer = new ServiceLayer();
        //Input validation
        await validate(serviceLayer, serviceLayer.removeOrder, { 'Order ID ': 'Order',  'Username ': 'User' })
    ...
    it('UnitTest addMovieOrder, removeOrder - Cinema System',async () => {
        ...
        await testCinemaFunctions(cinemaSystem, () => cinemaSystem.addMovieOrder(1,'',1,[],1));

        cinemaSystem = new CinemaSystem();
        await testCinemaFunctions(cinemaSystem, () => cinemaSystem.removeOrder(1,1));
    });
    ...
    it('UnitTest addMovieOrder - Inventory Management', async () => {
        ...
        result = await inventoryManagement.addMovieOrder(1,todayDate.toISOString(),1,[1],1);
    ...
    it('Integration addMovieOrder', async () => {
        ...
        serviceLayer.suppliers.set("Supplier", 1);
        serviceLayer.products.set("Movie", 1);
        await testCinemaFunctions(serviceLayer.cinemaSystem, () => serviceLayer.addMovieOrder('Order','date','Supplier', '["Movie"]', 'User'));
        ...
        result = await serviceLayer.addMovieOrder('Order','date','Supplier', '["Movie"]', 'User');
        expect(result).toBe("The order date is invalid");
        result = await serviceLayer.addMovieOrder('Order',todayDate.toISOString(),'Supplier', '["Movie"]', 'User');
        expect(result).toBe("The order added successfully");
    ...
    it('Integration removeOrder', async () => {
        ...
        await testCinemaFunctions(serviceLayer.cinemaSystem, () => serviceLayer.removeOrder('Order', 'User'));
