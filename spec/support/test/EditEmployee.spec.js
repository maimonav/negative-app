...
describe("EditEmployeeTest", () => {
  ...
  it("UnitTest-EditEmployee Test on class EmployeeManager", async () => {
    expect(
      await employeeManagemnt.editEmployee(
        -1,
        permissions,
        fname,
        lname,
    ...
    ).toEqual("The employee does not exist in the system.");
    expect(
      await employeeManagemnt.editEmployee(adminID, null, null, "dummy")
    ).toEqual("The Employee edited successfully");
  ...
  it("UnitTest-EditEmployee Test on class ServiceLayer", async () => {
    ...
    ).toEqual("The employee does not exist");
    expect(
      await servicelayer.editEmployee(
        adminUserName,
        "Director",
  ...
  it("integration-EditEmployee Test on class ServiceLayer", async () => {
    ...
    expect(
      servicelayer.cinemaSystem.employeeManagement.employeeDictionary.has(
        servicelayer.users.get(adminUserName)
      )
    ).toEqual(true);
    expect(
      servicelayer.cinemaSystem.employeeManagement.employeeDictionary.get(
        servicelayer.users.get(adminUserName)
      ).firstName
    ).toEqual("dummy");
    expect(
