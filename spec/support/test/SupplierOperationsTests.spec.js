...
describe("Supplier Operations Tests", () => {
    ...
    it('UnitTest addSupplier ,editSupplier, removeSupplier - Service Layer', async () => {
        ...
        //Input validation
        await validate(serviceLayer, serviceLayer.addNewSupplier, { 'Supplier Name ': 'Supplier', 'Contact Details ': '0500000000', 'Username ': 'User' })
        await validate(serviceLayer, serviceLayer.editSupplier, { 'Supplier Name ': 'Supplier', 'Contact Details ': '0500000000', 'Username ': 'User' })
        await validate(serviceLayer, serviceLayer.removeSupplier, { 'Supplier Name ': 'Supplier', 'Username ': 'User' })


        await testServiceFunctions(serviceLayer, async (name) => serviceLayer.addNewSupplier(name, '0500000000', 'User'),true);
        serviceLayer = new ServiceLayer();
        await testServiceFunctions(serviceLayer, async (name) => serviceLayer.editSupplier(name, '0500000000', 'User'));
        serviceLayer = new ServiceLayer();
        await testServiceFunctions(serviceLayer, async (name) => serviceLayer.removeSupplier(name, 'User'));
    ...
    it('UnitTest addSupplier, editSupplier, removeSupplier - Cinema System', async () => {
        ...
        await testCinemaFunctions(cinemaSystem, () => cinemaSystem.addNewSupplier(1, "", 1, 1));
        cinemaSystem = new CinemaSystem();
        await testCinemaFunctions(cinemaSystem, () => cinemaSystem.editSupplier(1, 1, "", 1, 1));
        cinemaSystem = new CinemaSystem();
        await testCinemaFunctions(cinemaSystem, () => cinemaSystem.removeSupplier(1, 1));
    ...
    it('UnitTest editSupplier, removeSupplier - Supplier', async () => {
        //edit
        let actualSupplier = new Supplier(1, "Supplier", "050");
        let expectedSupplier = new Supplier(1, "Sup", "052");
        expect(await actualSupplier.editSupplier("Sup", "052")).toBe("The supplier edited successfully");
        expect(expectedSupplier.equals(actualSupplier)).toBe(true);


        //remove
        expect(await expectedSupplier.removeSupplier()).toBe("The supplier removed successfully");
        expect(expectedSupplier.isSupplierRemoved != null).toBe(true);
        expect(await expectedSupplier.removeSupplier()).toBe("The supplier already removed");
    ...
    it('Integration addSupplier', async () => {
        ...
        await testCinemaFunctions(serviceLayer.cinemaSystem, async () => serviceLayer.addNewSupplier("Supplier", "050","User"));
        ...
        result = await serviceLayer.addNewSupplier("anotherSupplier", "050","User");
        expect(result).toBe("The supplier added successfully");
    ...
    it('Integration editSupplier', async () => {
        ...
        serviceLayer.users.set("User", 1);
        await testCinemaFunctions(serviceLayer.cinemaSystem, async () => serviceLayer.editSupplier("Supplier", "050", "User"));
    ...
    it('Integration removeSupplier', async () => {
        ...
        serviceLayer.suppliers.set("Supplier", 1);
        serviceLayer.users.set("User", 1);
        await testCinemaFunctions(serviceLayer.cinemaSystem, async () => serviceLayer.removeSupplier("Supplier", "User"));
        user = { isLoggedin: () => true, permissionCheck: () => true }
        serviceLayer.cinemaSystem.users.set(1, user);
...
async function testServiceFunctions(serviceLayer, method, isAdd) {
    ...
    if(isAdd)
        result = await method("anotherSupplier");
    else
        result = await method("Supplier");
