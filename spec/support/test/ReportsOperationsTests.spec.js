...
describe("Report Operations Unit Tests", () => {
  ...
  it("UnitTest addField , removeField - ReportController", async (done) => {
    ...
    setTimeout(async () => {
      ...
      setTimeout(async () => {
        let actualResult = await ReportController.getReport(
          "general_purpose_daily_report",
          todayDate
        );
        let expectedResult = {
        ...
        //remove
        result = await ReportController.removeFieldFromDailyReport(
          "Report Z taken"
        );
        expect(result).toEqual("The report field removed successfully");

        setTimeout(async () => {
          actualResult = await ReportController.getReport(
            "general_purpose_daily_report",
            todayDate
          );
  ...
  it("Integration getReport", async (done) => {
    ...
    //add report
    let serviceLayer = new ServiceLayer("mydbtest");
    await serviceLayer.initSeviceLayer("mydbtest");
    serviceLayer.users.set("User", 1);
    let user = { isLoggedin: () => true, permissionCheck: () => true };
    serviceLayer.cinemaSystem.users.set(1, user);
    ...
    await testCinemaFunctions(serviceLayer.cinemaSystem, () =>
      serviceLayer.getReport("test", "test", "User")
    );
    setTimeout(async () => {
  ...
  it("Integration addField", async (done) => {
    ...
    setTimeout(async () => {
      ...
      setTimeout(async () => {
        result = await serviceLayer.getReport(
          "general_purpose_daily_report",
          todayDate,
          "User"
        );
  ...
  it("Integration removeField", async (done) => {
    ...
    setTimeout(async () => {
      user = { isLoggedin: () => true, permissionCheck: () => true };
      serviceLayer.cinemaSystem.users.set(1, user);
      result = await serviceLayer.removeFieldFromDailyReport(
        "new_field",
        "User"
      );
