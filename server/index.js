...
app.get("/api/editEmployee", async(req, res) => {
    ...
    console.log("", userName, password, permission, firstName, lastName, contactDetails, user)
    const result = await service.editEmployee(userName, password, permission, firstName, lastName, contactDetails, user);
    res.send(JSON.stringify({ result }));
});
