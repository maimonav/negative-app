...
async function initDB(dbName, password) {
    ...
    try {
        ...
        return DataBase.errorHandler.apply(DataBase, [error, errId]);
    }
    ...
    try {
        ...
        if (dbName === undefined || dbName.toLowerCase() !== "mydbtest") {
            result = await setDestroyTimerForAllTables();
        ...
        result = await initGeneralReport();
        if (typeof result === "string") {
            DBlogger.info(
            ...
            afterBulkUpdate: async(user) => {
                if (
                    ...
                    await DataBase.singleUpdate(
                        "employee", { id: user.where.id }, { isEmployeeRemoved: true }
                    );
                }
            },
        },
