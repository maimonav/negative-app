...
class CafeteriaProduct extends Product {
  ...
  initCafeteriaProduct() {
    ...
    if (res === "error") return "The operation failed - DB failure";
    return "";
  }
  ...
  editProduct(categoryId, price, quantity, maxQuantity, minQuantity) {
    if (this.isNeedToUpdate(categoryId, false)) this.categoryId = categoryId;
    if (this.isNeedToUpdate(price, true)) this.price = price;
    if (this.isNeedToUpdate(quantity, true)) this.quantity = quantity;
    if (this.isNeedToUpdate(maxQuantity, true)) this.maxQuantity = maxQuantity;
    if (this.isNeedToUpdate(minQuantity, false)) this.minQuantity = minQuantity;
  ...
  removeProduct() {
    ...
    } else return "The product already removed";
  }
