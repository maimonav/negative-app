...
function getRecentEmailreq(auth, i) {
    ...
    gmail.users.messages.list({ auth: auth, userId: 'me', maxResults: 1, }, function(err, response) {
        ...
        if (response.data.resultSizeEstimate == 0) {
            //add to log file
            setTimeout(() => { getRecentEmailreq(auth, i + 1) }, 60000);
            return;
        }
        ...
        gmail.users.messages.get({ auth: auth, userId: 'me', 'id': message_id }, function(err, response) {
            if (err) {
                ...
                const emailDate = new Date(response.data.payload.headers[response.data.payload.headers.length - 14].value);
                emailDate.setHours(0, 0, 0, 0);
                const curDate = new Date();
                curDate.setHours(0, 0, 0, 0);
                const fromMail = response.data.payload.headers[response.data.payload.headers.length - 9].value;
                //if the mail is not relevant or not from today then delete him
                if (!fromMail.includes("info@eventbuzz.co.il") ||
...
function msgProcessor(data) {
    ...
    download(downloadUrl).then(setTimeout(() => { jsonToCSV(); }, 90000));
};
...
function jsonToCSV() {
    ...
    let fieldDelimiter = ',';
    let headers = ['name', 'date', 'location', 'totalExpectedTicket', 'totalActualTicket', 'totalIncomes', 'totalReturns', 'totalFees', 'totalForSeller', 'totalCash'];
...
async function eventbuzzScript() {
    return new Promise(async function() {
        ...
        await driver.findElement(By.id("email")).sendKeys("diklakat93@gmail.com", Key.RETURN);
        await driver.findElement(By.id("password")).sendKeys("נגטיב", Key.RETURN);
        await driver.findElement(By.xpath("/html/body/div[1]/form/div[3]/input")).sendKeys(Key.ENTER);



        var personalEaraBtn = await driver.wait(until.elementLocated(By.xpath("/html/body/div[1]/div[1]/div/div[1]/div[2]/img"), 2000));
        await personalEaraBtn.click();
        var reportsBtn = await driver.wait(until.elementLocated(By.xpath("//*[@id=\"menu-main-div\"]/main/div/div[2]/a[2]"), 2000));
        await reportsBtn.click();
        var send = await driver.wait(until.elementLocated(By.xpath("//*[@id=\"event_export_form\"]/form/div[1]/input"), 2000));
        await driver.findElement(By.id("email")).sendKeys("negativesystem123@gmail.com", Key.RETURN);
        let promises = await driver.findElement(By.xpath("//*[@id=\"event_export_form\"]/form/div[1]/input")).click();
        await driver.sleep(5000);
        driver.close();
        setTimeout(() => { downlowdReportMainFlow(); }, 120000);
    })
...
module.exports = { main, eventbuzzScript, downlowdReportMainFlow, json_ans };