...
class Order {
    ...
    async editOrder(date, supplierId, productsList) {
         ...
         * @returns {Promise(string)} Success or failure string
         **/
    async confirmOrder(productList) {
        ...
        if (numOfProduct > 0) {
            this.writeToLog('info', 'confirmOrder', 'No status was received for all order products');
        ...
        productList.forEach((product) => {
            if (this.productOrders.has(product.id)) {
                if (this.productOrders.get(product.id) instanceof CafeteriaProductOrder) {
                    let productAction = this.productOrders.get(product.id).getConfirmOrderDB(product.actualQuantity);
                    DBActionList.push(productAction[0]);
                    DBActionList.push(productAction[1]);
                } else if (this.productOrders.get(product.id) instanceof MovieOrder) {
                    let productAction = this.productOrders.get(product.id).getConfirmOrderDB(product.key, product.examinationRoom);
                    DBActionList.push(productAction[0]);
                    DBActionList.push(productAction[1]);
                }
            }
        ...
        productList.forEach((product) => {
            if (this.productOrders.has(product.id)) {
                if (this.productOrders.get(product.id) instanceof CafeteriaProductOrder) {
                    ...
                    this.productOrders.get(product.id).confirmOrder(product.key, product.examinationRoom);
                }
            }
        });
