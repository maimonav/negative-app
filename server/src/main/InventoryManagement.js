...
class InventoryManagemnt {
    ...
    async confirmOrder(orderId, productsList, recipientEmployeeId) {
        ...
        return this.orders.get(orderId).confirmOrder(productsList, recipientEmployeeId);
    }
        ...
        this.orders.forEach((order) => {
            ...
            if (!(orderDate > endDate) && !(orderDate < startDate)) {
                if (typeof isCafetriaProductOrder === 'undefined')
                    ...
                    if (isCafetriaProductOrder === true && order.hasCafetriaProductInOrder())
                        result.push({ title: order.name });
                    else if (isCafetriaProductOrder === false && order.hasMoviesInOrder())
                        result.push({ title: order.name });
                }
            }
        });
