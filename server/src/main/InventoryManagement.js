...
class InventoryManagemnt {
    ...
    async addCafeteriaProduct(productId, name, categoryID, price, quantity, maxQuantity, minQuantity) {
        if (this.products.has(productId)) {
            this.writeToLog('info', 'addCafeteriaProduct', "This product already exists");
        ...
        if (price <= 0) {
            this.writeToLog('info', 'addCafeteriaProduct', "Product price must be greater than 0");
            return "Product price must be greater than 0";
        }
        if (quantity < 0) {
            this.writeToLog('info', 'addCafeteriaProduct', "Product quantity must be greater or equal to 0");
            return "Product quantity must be greater or equal to 0";
        }
        if (typeof maxQuantity !== 'undefined' && typeof minQuantity !== 'undefined' && maxQuantity <= minQuantity) {
            this.writeToLog('info', 'addCafeteriaProduct', "Maximum product quantity must be greater than minimum product quantity");
        ...
        if (typeof result === 'string') {
            this.writeToLog('error', 'addCafeteriaProduct', "The operation failed - DB failure -" + result);
    ...
    async addCategory(categoryId, categoryName, parentID) {
        ...
        if (typeof result === 'string') {
            this.writeToLog("error", "addCategory", "The operation failed - DB failure" + result);
            return result;
        }
    ...
    async removeCategory(categoryId) {
        ...
        if (categoryToRemove.isCategoryRemoved !== null) {
            writeToLog('info', 'removeCategory', 'The category already removed')
