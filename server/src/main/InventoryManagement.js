...
class InventoryManagemnt {
    ...
    async editOrder(orderId, date, supplierId, productsList) {
        if (!this.orders.has(orderId)) {
            this.writeToLog("info", "editOrder", "Order " + orderId + " does not exsits.");
            return "Order " + orderId + " does not exsits.";
        }
        return this.orders.get(orderId).editOrder(date, supplierId, productsList);
    }

    async confirmOrder(orderId, productsList) {
        if (!this.orders.has(orderId)) {
            this.writeToLog("info", "confirmOrder", "Order " + orderId + " does not exsits.");
            return "Order " + orderId + " does not exsits.";
        }
        return this.orders.get(orderId).confirmOrder(productsList);
    }
