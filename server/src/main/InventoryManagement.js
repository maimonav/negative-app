...
class InventoryManagement {
  ...
  checkAndNotifyLowAndHighQuantity(productsList) {
    ...
    if (lowQuantityList.length > 0)
      NotificationController.notifyLowQuantity(lowQuantityList);
    if (highQuantityList.length > 0)
      NotificationController.notifyHighQuantity(highQuantityList);
  }
   ...
   * @param {String} pricestr The new price of the product (Optional parameter)
   * @param {String} quantitystr New quantity of product in stock (Optional parameter)
   * @param {String} maxQuantitystr New maximum limit of product quantity in stock (Optional parameter)
   * @param {String} minQuantitystr New minimum limit of product quantity in stock (Optional parameter)
   * @returns {Promise(string)} Success or failure string
   **/
  async editCafeteriaProduct(
    ...
    let price = null;
    if (!isNaN(pricestr)) price = parseInt(pricestr);
    let quantity = null;
    if (!isNaN(quantitystr)) quantity = parseInt(quantitystr);
    let maxQuantity = null;
    if (!isNaN(maxQuantitystr)) maxQuantity = parseInt(maxQuantitystr);
    let minQuantity = null;
    if (!isNaN(minQuantitystr)) minQuantity = parseInt(minQuantitystr);
    let result = await this.products
      .get(productId)
