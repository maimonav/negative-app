...
class ServiceLayer {
    ...
    async editOrder(orderName, dateSTR, supplierName, productsList, ActionIDOfTheOperation) {
        ...
        if (!this.orders.has(orderName)) {
            logger.info("ServiceLayer- editOrder - The order " + orderName + " doesn't exists");
            return " The order " + orderName + " doesn't exists";
        }
        let date;
        if (typeof dateSTR === 'string' && (new Date(dateSTR) !== "Invalid Date") && !isNaN(new Date(dateSTR)))
        ...
        productsList.forEach((product) => {
            if (!this.products.has(product.name)) {
                logger.info("ServiceLayer- editOrder - The product " + product.name + " doesn't exists");
        ...
        if (!this.users.has(ActionIDOfTheOperation)) {
            logger.info("ServiceLayer- editOrder - The user " + ActionIDOfTheOperation + " performing the operation does not exist in the system");
            return "The user performing the operation does not exist in the system";
        }

        return this.cinemaSystem.editOrder(this.orders.get(orderName), date, supplierID, productsList, this.users.get(ActionIDOfTheOperation));
    }
    ...
    async confirmOrder(orderName, productsList, ActionIDOfTheOperation) {
            ...
            if (!this.orders.has(orderName)) {
                logger.info("ServiceLayer- confirmOrder - The order " + orderName + " doesn't exists");
                return " The order " + orderName + " doesn't exists";
            }
            let problemticProductName;
            productsList.forEach((product) => {
            ...
            if (!this.users.has(ActionIDOfTheOperation)) {
                logger.info("ServiceLayer- confirmOrder - The user " + ActionIDOfTheOperation + " performing the operation does not exist in the system");
                return "The user performing the operation does not exist in the system";
            }

            return this.cinemaSystem.confirmOrder(this.orders.get(orderName), productsList, this.users.get(ActionIDOfTheOperation));
        }
        /**
